...
def generate_random_graph_erdos_renyi(
    ...
    output_graph = weight_graph_edges(
        output_graph, random_weights, min_weight, max_weight, seed=seed
    )
...
def generate_random_regular_graph(
    ...
    output_graph = weight_graph_edges(output_graph, random_weights, seed=seed)
...
def weight_graph_edges(
    ...
    if random_weights:
        weighted_edges = [
            (e[0], e[1], uniform(min_weight, max_weight)) for e in graph.edges
        ]
    else:
...
def generate_graph_from_specs(graph_specs: dict) -> nx.Graph:
    ...
    if type_graph == "erdos_renyi":
        ...
        graph = generate_random_graph_erdos_renyi(
            num_nodes, probability, random_weights, seed=seed
        )

    elif type_graph == "regular":
        degree = graph_specs["degree"]
        graph = generate_random_regular_graph(
            num_nodes, degree, random_weights, seed=seed
        )

    elif type_graph == "complete":
        graph = generate_random_graph_erdos_renyi(
            num_nodes, 1.0, random_weights, seed=seed
        )
