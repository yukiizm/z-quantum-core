...
from .wip.circuits import Circuit, RY, RX
...
def get_context_selection_circuit(
    ...
    for qubit, operator in term:
        if operator == "X":
            context_selection_circuit += Circuit([RY(-np.pi / 2)(qubit)])
        elif operator == "Y":
            context_selection_circuit += Circuit([RX(np.pi / 2)(qubit)])
        operator *= IsingOperator((qubit, "Z"))
...
class BasicEstimator(Estimator):
    ...
    def get_estimated_expectation_values(
        ...
        if shot_allocation_strategy == "uniform":
            ...
            _, frame, measurements_per_frame = estimate_nmeas_for_frames(
                self.prior_expectation_values  # (inside we use self.prior_expectation_values.targets)
            )
